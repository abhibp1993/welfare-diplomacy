...
Description: Given a message table generated by simulate_game.py containing the columns
    ...
    following columns:
        ...
        units,          # units discussed in the dialogue between powers 1, 2 during the phase.
        power1_units,   # units discussed by power 1 during the phase.
        power2_units    # units discussed by power 2 during the phase.
        other_units     # units discussed by other powers during the phase.
...
def load_csv(csv_file_path):
    ...
    assert set(df_.columns.tolist()) == {"phase", "power1", "power2", "dialogue", "locations"}, \
        f"Columns in the CSV file are not as expected. Expected: ['phase', 'power1', 'power2', 'dialogue', 'locations']"
...
def units(dialogue, locations, valid_units, model_name="gpt-4o-mini", temperature=0.7, top_p=1.0):
    """
    ...
    """
    system_prompt = f"""
    You are an expert in the game Diplomacy. 
    You have been asked to extract units mentioned in the given dialogue.
    Return a list of unique units mentioned in the dialogue, and nothing else.
    Expected output: ['A PAR', 'F MAR', ...]
    """

    user_prompt = f"""
        Extract units mentioned in the following Diplomacy game dialogue:
        {dialogue}
        
        If it helps, the locations mentioned in the dialogue are: 
        {locations}
    """
...
def extract_units(df, game):
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Extracting locations from dialogue"):
        ...
        units_in_dialogue = units(dialogue=row["dialogue"], locations=row["locations"], valid_units=set())

        # Add units to rows
        rows.append(units_in_dialogue)
...
if __name__ == '__main__':
    ...
    # Load the CSV file
    input_df = load_csv(csv_file_path / "5-light-haze-msg-table-locations.csv")

    # Extract locations
    output_df = extract_units(input_df, game_json)

    # Save to CSV
    output_df.to_csv(csv_file_path / "5-light-haze-msg-table-units.csv", index=False)
    logger.success(f"Extracted locations updated in {csv_file_path / '5-light-haze-msg-table-units.csv'}")
